class RangeList {
  ...
  reRank(list, i) {
    ...
    // Record the location and scope of the update
    let start = i;
    let end = i;
    let resMin = min;
    let resMax = max;

    // Determine if it overlaps with the previous range
    if (i !== 0) {
      const [preMin, preMax] = list[i - 1];
      if (min <= preMax) {
        // Coincident with the previous range, the pointer migrates
        start = i - 1;

        // The new range min value is adjusted to the previous range
        resMin = preMin;
      }
    }
    ...
    while (index < list.length) {
      ...
      // Look for the range of repetitions on the right
  ...
  add(range) {
    ...
    // If the length of the existing list is 0, push it directly
    if (len === 0) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (curMin < min) {
        ...
        // First insert the range into the array
  ...
  remove(range) {
    // Current range that you want to remove
    let [curMin, curMax] = range;
    let list = [...this.list];
    ...
    for (let i = 0, l = list.length; i < l; i++) {
      ...
      // The removal range intersects with the current range
      if (!(curMin > max || curMax < min)) {
        let arr = [];

        // The left side needs to be removed
        ...
        // The middle needs to be removed
        else if (curMin > min && curMax < max) {
          arr.push([min, curMin], [curMax, max]);
        }

        // The right needs to be removed
        else {
          if (min !== curMin) {
            arr.push([min, curMin])
          }
